...
const NavBar: React.FC = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 h-14 bg-white border-b border-gray-200 flex items-center px-6"
      ...
      <div className="flex items-center gap-2 mr-8 cursor-pointer" onClick={() => navigate('/')}>
        ...
        <span className="text-base font-semibold text-gray-900">BillBuddy</span>
      </div>
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <button
              ...
              className={`px-4 py-2 rounded-r-sm text-sm font-medium transition-colors cursor-pointer ${
                isActive
