...
const ImportResultCard: React.FC<{
  ...
  return (
    <div className="bg-white rounded-xl p-5 shadow-card space-y-3 relative animate-fade-in-up">
      {onDismiss && (
        <button
          onClick={onDismiss}
          className="absolute top-3 right-3 w-6 h-6 rounded-full hover:bg-gray-100 flex items-center justify-center text-gray-400 hover:text-gray-600 transition-colors text-xs cursor-pointer"
        >
          ✕
        </button>
      )}
